 ...
 * click-target list). The original engine maintains two lists:
 *   g_render_list — every drawable entity, walked each tick to advance
 *                   per-entity scripts (FUN_004012B0) + render (FUN_00406040)
 *   g_click_list  — clickable masks/hot-spots, walked by FUN_00406BB0 to
 *                   set DAT_0044988C (current hover obj_id)
 * Both use Entity._r0[0..3] as next ptr, [4..7] as prev ptr (1:1 with the
 * original 102-byte layout where +0/+4 are list pointers).
 * ========================================================================= */
Entity *g_render_list_head = NULL;       /* DAT_004502B8 (head only — list iteration via side-band tables below) */
Entity *g_click_list_head  = NULL;       /* DAT_004502A0 */

/* Side-band linked-list storage. The original engine stores next/prev
 * inside Entity._r0[0..7] (2× 4-byte pointers on 32-bit). On a 64-bit
 * host that overflows _r0 into flags1/flags2 and corrupts the entity.
 * Maintain external parallel arrays instead — same observable semantics. */
...
{
    /* Routing: which side-band table does this head refer to? */
    typeof(g_render_list_tbl) *t =
        (head == &g_render_list_head) ? &g_render_list_tbl : &g_click_list_tbl;
...
void UnlinkEntity(Entity *e)
{
    ...
    for (int pass = 0; pass < 2; ++pass) {
        typeof(g_render_list_tbl) *t = pass ? &g_click_list_tbl : &g_render_list_tbl;
        Entity **head = pass ? &g_click_list_head : &g_render_list_head;
        int idx = ent_list_index(e, (const Entity *const *)t->entities, t->count);
        if (idx < 0) continue;
        for (int i = idx; i < t->count - 1; ++i)
            t->entities[i] = t->entities[i + 1];
        --t->count;
        *head = t->count ? t->entities[0] : NULL;
    }
}

/* Clear both entity lists + the registration table. Called on scene exit
 * so each room starts with a fresh empty world (1:1 with the original's
 * FUN_00405F80 + FUN_00402DB0 cleanup pair invoked by LoadStage).
 *
 * T4 step 2-4: actor entities PERSIST across scene transitions (Ebek=1,
 * Fjej=2). Original engine spawns Ebek/Fjej once at game start; verb
 * scripts post-GO_EXIT use SET_ENTITY_XY on the same actor. T4 step 1
 * (atlas singleton in PreloadCommonAssets) was the prerequisite — with
 * shared atlas, actor's +0x28 intern slot stays valid across scenes.
 *
 * Preserve:
 *   - Render list: g_actor[0] + g_actor[1] entries
 *   - Click list: entries owned by kind=4 id=1/2 in update table
 *   - Update table: kind=2 id=1/2 (actor) + kind=4 id=1/2 (click payload)
 *   - Intern table: not reset (slots referenced by surviving entities
 *     stay valid; new spawns append fresh slots after g_ent_ptr_n) */
void EntityListClearAll(void)
{
    extern Entity *g_actor[2];

    /* --- Snapshot which click-list entries we want to keep (actor click
     * payloads at update_table kind=4 id=1/2).
     *
     * Owner check is critical: scripts also load masks at id=1/2 (e.g.
     * komnata 4 wejw_lpg.msk → id=2, 3 frames → 3 kind=4 click payloads
     * with verbs 7/8/9 for door exits). Those collide on id with the
     * Fjej actor click payload. A naive `kind==4 && id<=2` filter
     * preserves them across the scene change, leaving stale door-exit
     * click bboxes from komnata 4 active in komnata 5 — clicking inside
     * one returns verb 9 (transition cursor) even though the door is
     * in a totally different scene. Verify owner is actually an actor. */
    extern void *ent_ptr_resolve(uint32_t slot);
    Entity *keep_click[8]; int keep_n = 0;
    for (int r = 0; r < g_update_table_count && keep_n < 8; ++r) {
        if (g_update_table[r].kind != 4) continue;
        if (g_update_table[r].id != 1 && g_update_table[r].id != 2) continue;
    ...
    /* --- Click list: keep actor click payloads. --- */
    w = 0;
    ...
    /* --- Update table: keep actor entries and actor-owned click payloads.
     * Same owner-check reasoning as keep_click[]: id=1/2 also gets used
     * by per-scene masks (e.g. wejw_lpg.msk in komnata 4), so we have to
     * disambiguate by entity identity / owner, not just the id field. */
    w = 0;
    for (int r = 0; r < g_update_table_count; ++r) {
        int keep = 0;
        if (g_update_table[r].kind == 2 &&
            (g_update_table[r].e == g_actor[0] ||
             g_update_table[r].e == g_actor[1])) {
            keep = 1;
        } else if (g_update_table[r].kind == 4) {
            for (int i = 0; i < keep_n; ++i) {
                if (g_update_table[r].e == keep_click[i]) { keep = 1; break; }
            }
        }
    ...
    /* --- Walker state sync (T4 step 2): explicitly reset actor walker
     * fields so mid-walk state from old scene doesn't conflict with
     * new scene's entry-chain. Bytes:
     *   +0x32 (pc) +0x34/+0x36/+0x38 (osc counters)
     *   +0x3A (bits 0=walker_busy, 1=foot_anchor, 2=walker_active)
     *   +0x3C/+0x3E (delay) +0x40/+0x42 (osc indices)
     *   +0x44/+0x48 (walker fixed-point pos) +0x4C/+0x50 (walker remaining)
     * Leaves anchor (+0x22/+0x24) and atlas (+0x28) untouched. */
    for (int i = 0; i < 2; ++i) {
        ...
        *(uint16_t *)(eb + 0x36) = 0;
        *(uint16_t *)(eb + 0x38) = 0;
        *(uint8_t  *)(eb + 0x3A) &= (uint8_t)~5u;     /* clear bits 0, 2 */
        *(uint16_t *)(eb + 0x3C) = 0;
        *(uint16_t *)(eb + 0x3E) = 0;
        *(uint16_t *)(eb + 0x40) = 0;
        *(uint16_t *)(eb + 0x42) = 0;
    ...
    /* Intern table — NOT reset. Slots referenced by surviving actor
     * click payload (e.g. payload[+0x0A] → actor entity intern) must
     * stay valid. New scene's spawns append slots after g_ent_ptr_n. */

    /* Speech balloon kind=1 — destroyed with scene (not persisted). */
    extern Entity *g_speech_balloon;
    g_speech_balloon = NULL;
}

/* Iterators used by walker / renderer / ScriptCallEnableEnt. */
Entity *EntityListFirst(int click_list) {
    return click_list ? (g_click_list_tbl.count ? g_click_list_tbl.entities[0] : NULL)
                      : (g_render_list_tbl.count ? g_render_list_tbl.entities[0] : NULL);
}
Entity *EntityListAt(int click_list, int idx) {
    typeof(g_render_list_tbl) *t = click_list ? &g_click_list_tbl : &g_render_list_tbl;
