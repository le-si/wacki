...
/* DAT_0044346A — panel verb table. 6 slots, one per panel button. The
 * verb_id at slot i is what FUN_00407260 will report as the hovered
 * verb when the cursor is over panel button i. Initialised to 0x26
 * (neutral) by FUN_00402DB0; rewritten by FUN_004071F0 when the
 * verb-set page rotates (inventory scroll). */
uint16_t  g_panel_verb_tab[6] = { 0x26, 0x26, 0x26, 0x26, 0x26, 0x26 };
uint16_t  g_hover_panel_verb  = 0x26;   /* DAT_0044E450 */
/* DAT_0044988C — scene-hover verb (mouse over clickable mask / sprite in
 * the scene, NOT the bottom panel). 0x26 = no hover. Written by
 * FUN_00406BB0 (ClickHitTest); read by FUN_004067C0 (cursor state) to
 * choose between olowek/drzwi-star/kaseta/magnes slots. T31 v2 port. */
uint16_t  g_hover_scene_verb  = 0x26;   /* DAT_0044988C */

/* DAT_0044E5A4 / DAT_0044E5AC — cursor-redirect handshake. The original
 * sets DAT_0044E5AC = 1 when the mouse-down handler wants the next
 * panel hit to *consume* the hovered verb into the held-item slot
 * (i.e. "pick up" the verb). FUN_00407260 atomically clears both
 * flags and copies the panel verb into DAT_0044E5E8 (g_held_item). */
uint8_t   g_panel_cursor_redirect  = 0;
uint8_t   g_panel_cursor_redirect2 = 0;

/* PanelHitTest — 1:1 with FUN_00407260 @ 0x00407260.
 *
 *   void FUN_00407260(void) {
 *       DAT_0044e450 = 0x26;
 *       if ((DAT_0044e448 & 1) == 0) return;
 *       if (panel.draw_y[0] >= cursor.y) return;
 *       int local_x = cursor.x - panel.draw_x[0];
 *       for (int i = 0; i < 6; i++) {
 *           if (btn_x[i] < local_x && local_x < btn_x[i] + 0x28) {
 *               int local_y = cursor.y - panel.draw_y[0];
 *               if (btn_y[i] < local_y && local_y < btn_y[i] + 0x28
 *                && (DAT_0044e450 = verb_tab[i], DAT_0044e5a4 != 0))
 *               {
 *                   DAT_0044e5ac = 0;
 *                   DAT_0044e5a4 = 0;
 *                   DAT_0044e5e8 = DAT_0044e450;
 *               }
 *           }
 *       }
 *   }
 *
 * Button hitbox table (DAT_00443450, 6 short pairs) hardcoded:
 *   { (300,20), (345,20), (390,20), (435,20), (480,20), (525,20) }
 * Each cell is 40 (=0x28) wide and 40 tall, all on the top row of the
 * panel (panel-local Y=20). */
extern AnimAsset *g_panel_asset;
extern int16_t    s_mouse_x;
extern int16_t    s_mouse_y;
extern uint16_t   g_held_item;          /* DAT_0044E5E8 */

void PanelHitTest(void)
{
    g_hover_panel_verb = 0x26;                  /* neutral default */
    if (!(g_settings_anim_active & 1)) return;  /* panel not visible */
    if (!g_panel_asset || !g_panel_asset->off_drawX || !g_panel_asset->off_drawY)
        return;
    ...
    /* DAT_00443450 — 6 panel-local button (x, y) pairs.
     * Verified from raw bytes 2c 01 14 00 59 01 14 00 86 01 14 00
     *                         b3 01 14 00 e0 01 14 00 0d 02 14 00 */
...
/* =========================================================================
 * Item-name voice-over (FUN_004021c0 + FUN_00402230 1:1)
 *
 * Original engine: hovering an inventory item in the bottom panel for ~2s
 * fires a female-voice WAV that names the item ("magnes", "drut", …).
 *
 * Table lives in Item.scr (loaded by InitializeGameSubsystems at boot)
 * with the format:
 *     [N]name.wav
 *     [N]name.wav
 *     ...
 * where N is 1-based. Verb id 0x2A maps to item index 1 (= table slot 0),
 * 0x2B → 2 (slot 1), etc., up to 0xB6 → 141 (slot 140).
 *
 * Original storage: 16-byte fixed-length entries indexed by (verb - 0x2A).
 * ========================================================================= */
#define ITEM_NAME_MAX        256       /* max items (original was unbounded) */
#define ITEM_NAME_FILENAME   16        /* 1:1 with FUN_00402230's 0x10 stride */
static char g_item_wav[ITEM_NAME_MAX][ITEM_NAME_FILENAME] = {{0}};
static int  g_item_wav_count = 0;

/* LoadItemNamesTable — 1:1 with FUN_00402230 @ 0x00402230.
 * Reads Item.scr, parses `[N]filename` lines, stores filename at index N-1.
 * Returns the highest N seen (= original `this+4` count). */
int LoadItemNamesTable(void)
{
    void *raw = NULL; uint32_t sz = 0;
    ...
    /* Single-pass parser — for each `[N]` block, read decimal N, then
     * after `]` skip whitespace, then copy up to 15 chars of filename
     * (stop on whitespace / control byte). 1:1 with original loop. */
    while (p < end) {
        while (p < end && *p != '[') ++p;
        if (p >= end) break;
        ++p;                                /* skip '[' */
        ...
        /* skip whitespace */
        while (p < end && *p <= ' ') ++p;
        /* copy filename, stop on whitespace/control */
        int slot = n - 1;
        int copied = 0;
...
/* ItemHoverDwellTick — 1:1 with the inventory-hover branch inside
 * ProcessGameFrameTick @ 0x004028B4..0x004028DD.
 *
 * Call once per frame, AFTER PanelHitTest. State machine:
 *   - hover_panel_verb out of [0x2A, 0xB6] → reset last, arm timer to
 *     now + 2000 ms (= 200 ticks at the original's 10-ms ISR rate).
 *   - in range AND index != last AND dwell elapsed → PlaySfx + arm timer
 *     + remember index (don't refire until index changes or dwell elapses
 *     again). */
extern void PlaySfx(const char *name);
extern uint16_t g_hover_panel_verb;
extern uint32_t g_tick_counter;       /* milliseconds */
void ItemHoverDwellTick(void)
{
    ...
    /* Original `if (DAT_00451470 != '\x01')` cursor-state gate omitted —
     * we don't model the 10-state cursor machine, but the verb-range
     * check below already filters out non-inventory hovers. */
    if (verb < 0x2A || verb > 0xB6) {
        s_last_item   = -1;
        s_dwell_until = g_tick_counter + 2000;
        return;
    }
    int idx = (int)(verb - 0x2A);
...
/* =========== inventory + panel page rotation ============================
 *
 * The original engine stores the player's inventory in DAT_00443332 — a
 * flat array of 60 ushort verb_ids. Slot value 0x26 = "empty"; any other
 * value in 0x29..0xB6 = a held item's panel verb_id. The 60 slots are
 * paginated 6-per-panel-row; the current page index is DAT_0044E458.
 *
 * Same storage backs the save game (see WackiSlot.scene_snapshot). The
 * port already declares it as g_scene_snapshot[30] (uint32_t for save
 * IO); we re-cast it here as `g_inventory` (uint16_t *) for the panel
 * code that thinks in slots.
 *
 * The dialog system reuses these data structures: op 0x19 fills 6 slots
 * with choice verbs, op 0x1A page-swaps them onto the panel, and the
 * user-click then routes through the same DispatchClickEvent path as
 * for regular inventory.
 */
uint16_t  g_panel_page_idx = 0;         /* DAT_0044E458 — current page */
...
/* ResetInventory — 1:1 with FUN_00405630 @ 0x00405630.
 *
 *   puVar2 = &DAT_00443332;
 *   for (iVar1 = 0x1e; iVar1 != 0; --iVar1) {
 *       *puVar2 = 0x260026;       // two slots per dword
 *       puVar2 = puVar2 + 1;
 *   }
 *   DAT_0044e458 = 0;
 */
void ResetInventory(void)
{
    ...
    for (int i = 0; i < 60; ++i) inv[i] = 0x26;
    g_panel_page_idx = 0;
}

/* PanelPageSwap — 1:1 with FUN_004071F0 @ 0x004071F0.
 *
 *   if ((DAT_0044e448 & 1) == 0 || DAT_0044e6ac == 0) return;
 *   uVar3 = 0; sVar4 = DAT_0044e458 * 6; iVar2 = 0;
 *   do {
 *       DAT_00452e70[i*2] = DAT_0044346a[i*2];      // backup current
 *       DAT_0044346a[i*2] = (&DAT_00443332)[i + page*6];  // new
 *       ++uVar3; iVar2 += 2;
 *   } while (uVar3 < 6);
 *   DAT_004527d0 = 1;          // dirty: redraw panel
 *   DAT_00452e7c = 0;
 *   DAT_00452e80 = 4;
 *
 * DAT_0044E6AC is the inventory items atlas (przedm.wyc) — its presence
 * means "icon table is ready", so the guard reads "only swap if panel is
 * visible AND we have icons to draw". g_items_atlas (loaded by
 * PreloadCommonAssets) plays that role.
 *
 * The backup at DAT_00452E70 isn't used elsewhere in the original
 * decompile we have visibility on — looks like dialog state stash. We
 * keep one. The "redraw panel" flag (DAT_004527D0) marks the panel for
 * re-blit; in our port we always redraw, so the flag is set but unread.
 */
uint16_t g_panel_verb_tab_backup[6] = { 0x26, 0x26, 0x26, 0x26, 0x26, 0x26 };
uint8_t  g_panel_redraw = 0;            /* DAT_004527D0 */

void PanelPageSwap(void)
{
    if (!(g_settings_anim_active & 1) || !g_items_atlas) return;
    uint16_t *inv = Inventory();
    int page_base = g_panel_page_idx * 6;
    for (int i = 0; i < 6; ++i) {
...
/* InventoryPagePrev — 1:1 with FUN_004055B0 @ 0x004055B0.
 *
 *   if (DAT_0044e458 != 0) {
 *       --DAT_0044e458;
 *       return 1;
...
{
    if (g_panel_page_idx != 0) { --g_panel_page_idx; return 1; }
    return 0;
}

/* InventoryPageNext — 1:1 with FUN_004055D0 @ 0x004055D0.
 *
 * Walks forward in the inventory array starting at page+1, looking for
 * the next non-empty slot. If found, advance page index by 1 and return 1.
 * Capped at 9 pages * 6 slots = 54 (less than 60 — last slot reserved?).
 */
int InventoryPageNext(void)
{
    if (g_panel_page_idx >= 9) return 0;
    uint16_t *inv = Inventory();
    int cursor = (g_panel_page_idx + 1) * 6;
    while (cursor < 60) {
        if (inv[cursor] != 0x26) { ++g_panel_page_idx; return 1; }
        ++cursor;
    }
    return 0;
}

/* InventoryPageCollapse — 1:1 with FUN_004056D0 @ 0x004056D0.
 *
 * After removing an item, walks backwards from the current page until
 * a page with at least one non-empty slot is found, or page 0 reached.
 * No return value.
 */
void InventoryPageCollapse(void)
{
    ...
    while (g_panel_page_idx != 0) {
        int base = g_panel_page_idx * 6;
        int empty = 1;
        for (int i = 0; i < 6; ++i) {
            if (inv[base + i] != 0x26) { empty = 0; break; }
        }
        if (!empty) return;
        --g_panel_page_idx;
    }
...
/* InventorySetPageForItem — 1:1 with FUN_00405680 @ 0x00405680.
 *
 * Given an item verb_id (in range 0x29..0xB6), search the inventory
 * array for a slot containing that verb and set the page index to the
 * slot's page (slot / 6). Used after picking up an item to make sure
 * the just-acquired item is visible on the panel.
 *
 * Original uses signed-magic divide-by-6 (* 0x2aaaaaab >> 32) for the
 * page calculation; portable C just uses /6.
 */
void InventorySetPageForItem(uint16_t item_verb)
{
    if ((uint16_t)(item_verb - 0x29) >= 0x8E) return;
    uint16_t *inv = Inventory();
    for (int i = 0; i < 60; ++i) {
        if (inv[i] == item_verb) {
            g_panel_page_idx = (uint16_t)(i / 6);
            return;
        }
    }
}

/* InventoryAddItem — 1:1 with FUN_00405730 @ 0x00405730.
 *
 * Add an item to the inventory. Searches for an existing slot holding
 * the same verb_id; if not found, takes the first 0x26 (empty) slot.
 * On success, writes the item entity's panel verb_id into the slot
 * (read from g_entity_state[(item-0x29)*8/2] = ushort at byte offset 0
 * of the per-item state), and marks the entity as in-inventory by
 * writing 0xFFFF at byte offset +2 of the same entry.
 *
 * Returns 1 on success, 0 if item out of range or inventory full.
 *
 * Entity-state layout (per-item, 8-byte stride, indexed by item-0x29):
 *   +0  ushort  panel_verb_id  (what shows up in inventory slot)
 *   +2  ushort  in_inventory   (0=no, 0xFFFF=yes)
 *   +4  ushort  state_a
 *   +6  ushort  state_b
 *
 * Total: 0x8E items × 8 bytes = 0x470 bytes = matches the C-side
 * `uint32_t g_entity_state[0x11C]` (= 0x470 bytes).
 */
int InventoryAddItem(uint16_t item_verb)
{
    if ((uint16_t)(item_verb - 0x29) >= 0x8E) return 0;
    uint16_t *inv = Inventory();
    int slot = -1;
    for (int i = 0; i < 60; ++i) {
    ...
    if (slot < 0) {
        for (int i = 0; i < 60; ++i) {
            if (inv[i] == 0x26) { slot = i; break; }
        }
    }
    if (slot < 0) return 0;                  /* inventory full */
    int idx = (item_verb - 0x29);
    uint16_t *es = (uint16_t *)g_entity_state;
    uint16_t panel_verb = es[idx * 4 + 0];   /* +0 panel_verb_id */
    es[idx * 4 + 1]    = 0xFFFF;             /* +2 mark in-inventory */
    inv[slot] = panel_verb;
    return 1;
}

/* InventoryRemoveItem — 1:1 with FUN_004057C0 @ 0x004057C0.
 *
 * Remove an item from the inventory. Finds slot holding `item_verb`,
 * shifts all subsequent slots one position left, sets the last slot
 * (index 59) to 0x26 (empty). The original tail-fill writes 0x26 to
 * DAT_004433A8 (= DAT_00443332 + 0x76 = byte offset 118 = slot 59 as
 * ushort).
 *
 * Returns 1 if found+removed, 0 if not found.
 */
int InventoryRemoveItem(uint16_t item_verb)
{
    ...
    for (int i = 0; i < 60; ++i) {
        if (inv[i] == item_verb) {
            if (i < 59) memmove(&inv[i], &inv[i + 1], (size_t)(59 - i) * sizeof(uint16_t));
            inv[59] = 0x26;
            return 1;
        }
    }
...
/* InventoryDropItem — 1:1 with FUN_00405850 @ 0x00405850.
 *
 *   piVar1 = FUN_00404C30(item_verb);      // FindEntityByVerbId
 *   if (piVar1 != NULL) {
 *       piVar1[+0x2C] = 0;                   // clear script ptr
 *       piVar1[+0x22] = 1000;                // anchor X off-screen
 *       piVar1[+0x0A] = 1000;                // draw X off-screen
 *   }
 *
 * Strips the item entity of its bytecode + parks it off-screen so the
 * renderer doesn't draw it any more. Used to "consume" an item after
 * it's been added to the inventory or used. */
...
void InventoryDropItem(uint16_t item_verb)
{
    ...
    uint8_t *eb = (uint8_t *)e;
    *(uint32_t *)(eb + 0x2C) = 0;
    *(int16_t  *)(eb + 0x22) = 1000;
    *(int16_t  *)(eb + 0x0A) = 1000;
}

/* InventoryHasItem — 1:1 with FUN_00405650 @ 0x00405650.
 *
 *   uVar1 = 0;
 *   do {
 *     if ((&DAT_00443332)[uVar1 & 0xffff] == param_1) return 1;
 *     uVar1 = uVar1 + 1;
 *   } while ((ushort)uVar1 < 0x3c);
 *   return 0;
 *
 * Returns 1 if `item_verb` is currently in any of the 60 inventory
 * slots, 0 otherwise. Used by op 0x23 to set g_return_reg as a
 * condition for subsequent IFs ("if has item, then ...").
 */
...
/* Deferred click-event queue — 1:1 with DAT_0044A1A0 / DAT_0044A1C8.
 *
 * Op 0x22 in the per-entity VM enqueues here instead of dispatching
 * synchronously; FlushQueuedClicks drains via DispatchClickEvent at the
 * end of each frame tick. That matches the original engine where
 * ProcessGameFrameTick's tail loop drains the queue AFTER all per-entity
 * ticks have completed — so entity-list mutations from scripts don't
 * corrupt the iteration in progress. */
#define CLICK_QUEUE_MAX 10
static struct { uint16_t obj, verb; } g_click_queue[CLICK_QUEUE_MAX];
static int                            g_click_queue_count = 0;

void EnqueueClickEvent(uint16_t obj, uint16_t verb)
{
    if (g_click_queue_count >= CLICK_QUEUE_MAX) return;
    g_click_queue[g_click_queue_count].obj  = obj;
    g_click_queue[g_click_queue_count].verb = verb;
    ++g_click_queue_count;
}

void FlushQueuedClicks(void)
{
    extern void DispatchClickEvent(uint16_t obj, uint16_t verb);
    int n = g_click_queue_count;
    g_click_queue_count = 0;
    for (int i = 0; i < n; ++i)
        DispatchClickEvent(g_click_queue[i].obj, g_click_queue[i].verb);
}
 ...
 *
 * 1:1 port of FUN_00410D30 (Enqueue) + FUN_00410D20 (Reset) @ 0x00410D20.
 *
 * Original layout: DAT_00479380 is a 16-entry array, each entry 16 bytes
 * (8 ushorts), populated by the script op 0x41 SOUND_PLAY:
 *   +0  (u16) world_x       (= reg_id, the x coord of source)
 *   +2  (u16) world_y       (= a1, y coord of source)
 *   +4  (u16) volume        (= a2, base volume 0..255)
 *   +6  (u16) sound_id_lo   (low byte of u32 ID at pc+8)
 *   +8  (u16) sound_id_md   (mid byte of same)
 *   +A  (u16) sound_id_hi   (high byte of same)
 *
 * DAT_00479480 is the queue count (max 16). Reset zeroes the count;
 * Enqueue appends a new entry if there's room.
 *
 * FUN_00410DA0 (the consumer) walks the queue and computes a stereo pan
 * + volume falloff against a listener position; the result is a packed
 * 24-bit (L, C, R) byte triple. The current SDL2 build doesn't have
 * a stereo SFX mixer wired (audio.c uses a single mono device), so the
 * computation is informational only — we keep the queue state in sync
 * with the original so any future stereo mixer can read it.
 */
...
/* SoundQueueReset — 1:1 with FUN_00410D20.
 *   DAT_00479480 = 0; */
...
/* SoundQueueEnqueue — 1:1 with FUN_00410D30.
 *   if (DAT_00479480 < 0x10) {
 *     slot = &queue[count];
 *     slot[0] = param_1; slot[1] = param_2; slot[2] = param_4;
 *     slot[3] = (param_3 >> 0)  & 0xFF;
 *     slot[4] = (param_3 >> 8)  & 0xFF;
 *     slot[5] = (param_3 >> 16) & 0xFF;
 *     ++DAT_00479480;
 *   } */
void SoundQueueEnqueue(int16_t x, int16_t y, uint32_t sound_id, uint16_t volume)
{
    ...
    e->x = x; e->y = y; e->volume = volume;
    e->sound_id[0] = (uint8_t)(sound_id      );
    e->sound_id[1] = (uint8_t)(sound_id >>  8);
    e->sound_id[2] = (uint8_t)(sound_id >> 16);
}

/* T36 — SoundQueueMixForListener — port of FUN_00410DA0 distance + pan calc.
 *
 * Original (Ghidra @ 0x00410DA0):
 *   if (queue empty) return 0x808080;             // identity L=C=R=0x80
 *   L = C = R = 0;
 *   for (each queued source) {
 *     dx = |listener_x - src.x|; dy = |listener_y - src.y|;
 *     d  = sqrt(dx² + 2*dy²) / src.volume;         // weighted Euclidean
 *     if (d > DAT_004220e0) d = DAT_004220e0;      // clamp to max range
 *     // fcos(d) -> 3 ftol() conversions for L/C/R contributions
 *     L += L_contrib; C += C_contrib; R += R_contrib;
 *   }
 *   clamp each to 0xFF;
 *   return (R << 16) | (C << 8) | L;
 *
 * Port approximation: equal-power pan law per source. src.volume as
 * 0..255 amplitude, signed dx as pan input (neg = left, pos = right).
 * Precise constants from the original decompile are lost (the ftol()
 * triple); the behavioural envelope (left-of-listener quieter on R
 * channel etc.) holds and matches the API contract callers expect.
 * Empty queue still returns 0x808080 identity gain (1:1 with Ghidra). */
uint32_t SoundQueueMixForListener(int16_t listener_x, int16_t listener_y)
{
    if (s_sound_queue_count == 0)
        return 0x00808080u;                       /* identity (per original) */

    int L = 0, C = 0, R = 0;
    /* DAT_004220E0 = max-distance clamp constant. Original is a float;
     * port uses fixed-point with D_MAX = 240 px (further than diagonal
     * of 640×480 stage * 0.5 — anything more than half the screen
    ...
    for (uint16_t i = 0; i < s_sound_queue_count; ++i) {
        ...
        int adx = dx < 0 ? -dx : dx;
        int ady = dy < 0 ? -dy : dy;
        int dist_sq = adx*adx + 2*ady*ady;
        int v       = (int)s_sound_queue[i].volume;
        if (v <= 0) continue;
        if (dist_sq > D_MAX * D_MAX) dist_sq = D_MAX * D_MAX;
        int contrib = (v * 256) / (dist_sq + 256);     /* 0..255-ish */
        if (contrib > 255) contrib = 255;

        int pan = (dx * 255) / D_MAX;                  /* -255..+255 */
        if (pan < -255) pan = -255;
        if (pan >  255) pan =  255;
        int gL = ((255 - pan) * contrib) / 510;
        int gR = ((255 + pan) * contrib) / 510;
        int gC = (contrib * (D_MAX - ady)) / (D_MAX + ady + 1);
...
extern void PlaySfxPanned(const char *wav, uint8_t gain_l, uint8_t gain_r);

/* T109 audit (2026-05-27) — `s_sound_table[]` retired. Earlier port had
 * a hardcoded id→wav lookup table here as a "best-effort fallback" for
 * op 0x41 direct playback. After RE'ing the original:
 *
 *   case 0x41: FUN_00410D30(uVar29, a1, *(u32)(pc+8), a2);
 *
 * FUN_00410D30 is JUST the positional queue enqueue — it stores (source_x,
 * source_y, sound_id, volume) into DAT_00479380[]. It does NOT trigger
 * playback. Per-frame, FUN_00410DA0 (= SoundQueueMixForListener in port)
 * reads the queue + computes aggregate L/C/R pan based on listener vs all
 * queued sources, returned via packed uint32. FUN_00410370 then sets
 * master volume from that.
 *
 * Actual WAV playback in the original comes from FRAME-LEVEL [sampl] tags
 * in Wacky.scr (= per-entity animation triggers), already ported via
 * TriggerFrameSfx. Op 0x41 just provides the POSITIONAL state for the
 * aggregate pan — no direct playback.
 *
 * So the "missing s_sound_table" is a non-bug: original doesn't have one
 * because op 0x41 doesn't need it. Removed the unnecessary fallback path. */

void ScriptCallSoundPlay(uint16_t id, uint16_t a, uint32_t b, uint16_t c)
{
    /* 1:1 with op 0x41:
     *   FUN_00410D30(reg_id, a1, *(u32)(pc+8), a2);
     * Map to: source_x=reg_id, source_y=a1, sound_id=u32, volume=a2.
     * The original treats source coords as 16-bit; reuse here. */
    SoundQueueEnqueue((int16_t)id, (int16_t)a, b, c);

    /* T36 — compute stereo pan via SoundQueueMixForListener relative to
     * active actor's anchor position. The result here is informational
     * only — the original doesn't trigger direct playback from op 0x41
     * either (see T109 audit comment above), so we just log it. */
    int16_t lx = WACKI_SCREEN_W / 2, ly = WACKI_SCREEN_H / 2;
    ...
    fprintf(stderr, "[script] sound enqueue id=0x%04x src=(%d,%d) vol=%u "
                    "pan=L%u/C%u/R%u\n",
            id, (int)id, (int)a, c,
...
{
    /* 1:1 with op 0x42:
     *   FUN_00410D20();   // reset positional queue
     * Also stop the music player (covers the menu-style usage). */
    SoundQueueReset();
    StopMenuMusic();
}

/* ScriptCallAnimActor removed — ops 0x10/0x11/0x12 now call
 * ActorWalkToBlocking directly (1:1 port of FUN_004061D0 walker). */
